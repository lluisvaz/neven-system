...
import { CreditCard, Calendar, HelpCircle, FileText, TrendingUp, Download, Eye, Clock, Activity } from "lucide-react";
...
const activeTickets = [
  { id: "TKT-1042", title: "Dúvida sobre fechamento contábil", status: "Em andamento", updated: "Hoje às 10:30" }
];
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Bem-vindo(a) ao portal do cliente, João.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-4 border-y border-border">
        <div className="flex flex-col space-y-1">
          ...
          <span className="text-2xl font-mono text-foreground">Enterprise</span>
        </div>
        <div className="flex flex-col space-y-1">
          <div className="flex items-center text-muted-foreground gap-2">
          ...
          <span className="text-2xl font-mono text-foreground">01/05/2026</span>
        </div>
        <div className="flex flex-col space-y-1">
          <div className="flex items-center text-muted-foreground gap-2">
          ...
          <span className="text-2xl font-mono text-foreground">R$ 15.5k</span>
        </div>
        <div className="flex flex-col space-y-1">
          <div className="flex items-center text-muted-foreground gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/portal/invoices"><Button variant="link" size="sm" className="h-auto p-0 text-xs">Ver histórico</Button></Link>
          </div>
            ...
            {recentInvoices.map((inv, i) => (
              <div key={i} className="flex items-center justify-between p-4 border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                ...
                <div className="text-right">
                  ...
                  <Badge variant={inv.status === 'Pago' ? 'default' : 'secondary'} className="mt-1 rounded-sm font-mono text-[10px] uppercase tracking-wider px-2 py-0.5">
                    {inv.status}
                  </Badge>
                </div>
              </div>
        ...
        <div className="space-y-4">
            ...
            <div className="border border-border rounded-sm overflow-hidden bg-card p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <Link href="/portal/tickets"><Button variant="link" size="sm" className="h-auto p-0 text-xs mt-2 text-accent">Acompanhar</Button></Link>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="text-right">
            <Link href="/portal/documents"><Button variant="link" size="sm" className="h-auto p-0 text-xs">Ver todos os documentos</Button></Link>
          </div>
        </div>
      </div>
    </div>

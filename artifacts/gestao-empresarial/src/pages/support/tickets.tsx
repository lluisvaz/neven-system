  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button size="sm" className="rounded-sm w-fit"><Plus className="w-3.5 h-3.5 mr-2" />Novo Ticket</Button>
      </div>
          ...
          <Table>
            <TableHeader className="bg-muted/50">
              <TableRow className="hover:bg-transparent">
                <TableHead className="font-medium text-xs uppercase tracking-wider h-10 w-24 hidden sm:table-cell">Ticket</TableHead>
                <TableHead className="font-medium text-xs uppercase tracking-wider h-10">Assunto</TableHead>
                <TableHead className="font-medium text-xs uppercase tracking-wider h-10 hidden md:table-cell">Cliente</TableHead>
                <TableHead className="font-medium text-xs uppercase tracking-wider h-10 text-center">Prioridade</TableHead>
                <TableHead className="font-medium text-xs uppercase tracking-wider h-10 w-24 text-center hidden sm:table-cell">SLA</TableHead>
            ...
            <TableBody>
              {tickets.map(t => (
                <TableRow key={t.id} className={`hover:bg-muted/30 transition-colors ${["Resolvido", "Fechado"].includes(t.status) ? "opacity-60" : ""}`}>
                  <TableCell className="py-3 text-sm font-mono hidden sm:table-cell">
                  ...
                  <TableCell className="py-3">
                    ...
                    <div className="text-xs text-muted-foreground mt-0.5 hidden sm:block">Criado em {t.created} • Atualizado em {t.updated}</div>
                  </TableCell>
                  <TableCell className="py-3 text-sm hidden md:table-cell">{t.client}</TableCell>
